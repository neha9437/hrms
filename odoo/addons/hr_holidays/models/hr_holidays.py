...
class Holidays(models.Model):
    ...
    def _check_state_access_right(self, vals):
        if vals.get('state') and vals['state'] not in ['draft', 'confirm', 'cancel'] and not self.env['res.users'].has_group('hr_holidays.group_hr_holidays_user') and not self.env.user.has_group('hr_holidays.group_hr_holidays_manager') and not self.env.user.has_group('orient_leave_management.group_hr_holidays_reporting'):
            return False
        return True
    ...
    def _check_security_action_approve(self):
        if not self.env.user.has_group('hr_holidays.group_hr_holidays_user') and not self.env.user.has_group('hr_holidays.group_hr_holidays_manager') and not self.env.user.has_group('orient_leave_management.group_hr_holidays_reporting'):
