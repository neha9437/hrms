...
class Applicant(models.Model):
    ...
    #         list_id = self.env['interview.list'].search([('id','=',interview_id)])
    #         print(list_id,'list_id')
    #         job_posting = list_id.job_posting
    #         print(job_posting,'=====')
    #         job_id = list_id.job_posting.designation.id
    #         department_id = list_id.job_posting.department_id.id
    #         process = job_posting.process.id
    #         print(process,department_id,job_id,'process')

    #         stages = self.env['round.child'].search([('round_id','=',process)])
    #         stages_list.append(stages)
    #     print(stages,'stages')

    #     for each in stages:
    #         print(each.stage_id,'each')

    #         # if job_id.jobprocessone2many:
    #         #     for each in job_id.jobprocessone2many:
    #         #         print(each.name,'each')
